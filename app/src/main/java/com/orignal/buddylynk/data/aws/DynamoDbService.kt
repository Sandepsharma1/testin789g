...
object DynamoDbService {
    ...
    // Event operations - IN-MEMORY storage (for demo)
    private val eventsCache = mutableListOf<Event>()
    
    suspend fun getEvents(): List<Event> {
        android.util.Log.d("DynamoDbService", "getEvents() called, returning ${eventsCache.size} events")
        return eventsCache.toList()
    }
    
    suspend fun createEvent(event: Event): Boolean {
        android.util.Log.d("DynamoDbService", "createEvent() called: ${event.title}")
        eventsCache.add(0, event) // Add to front
        android.util.Log.d("DynamoDbService", "Event created! Total events: ${eventsCache.size}")
        return true
    }
