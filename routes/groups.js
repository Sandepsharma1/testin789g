...
router.get('/', verifyToken, async (req, res) => {
    try {
        const result = await docClient.send(new ScanCommand({
            ...
            // Check both 'members' (old schema) and 'memberIds' (new schema)
            FilterExpression: 'creatorId = :uid OR contains(memberIds, :uid) OR contains(members, :uid)',
            ExpressionAttributeValues: { ':uid': req.userId }
        }));

        const groups = (result.Items || []).map(convertGroupMediaUrls);
        console.log(`[Groups] Returning ${groups.length} groups for user ${req.userId}`);
        res.json(groups);
    } catch (err) {
