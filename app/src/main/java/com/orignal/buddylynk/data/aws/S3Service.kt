...
object S3Service {
    ...
    /**
     * Core upload function - uploads a Uri using pre-signed URL (Streaming)
     */
    private suspend fun uploadMedia(
        ...
        try {
            ...
            val presignJson = presignResult.getOrNull() ?: return@withContext null
            val uploadUrl = presignJson.optString("uploadUrl", "").ifEmpty { presignJson.optString("presignedUrl", "") }
            val fileUrl = presignJson.optString("fileUrl", "")
            ...
            Log.d(TAG, "Got pre-signed URL, uploading via streaming: $mediaUri")
            
            // Upload to S3 using pre-signed URL (Streaming)
